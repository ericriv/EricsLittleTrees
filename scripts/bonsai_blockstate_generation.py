...
bonsai_model_variants = ["base", "first"]
    ...
    for content in contents_variants:
        for wire in wire_variants:
            for bonsai in bonsai_variants:
                for bonsai_model in bonsai_model_variants:
                    rotation = 0
                    if facing == "east":
                        rotation = 90
                    elif facing == "south":
                    ...
                    if content == "empty":
                        ...
                        if bonsai_model == "base":
                            model = f"ericslittletrees:block/pottedbonsai/basic/{model_name}_bonsai"
                        elif bonsai_model == "first":
                            model = f"ericslittletrees:block/pottedbonsai/first/{model_name}_bonsai"
                    elif wire == "true":
                        model_name = content
                    ...
                    json_data["variants"][f"is_bonsai={bonsai},bonsai_model={bonsai_model},has_wire={wire},contents={content},facing={facing}"] = {
                        "model": model,
                        "y": rotation
                    }
